...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Kassa kassa1 = new Kassa(store, 1);
        Kassa kassa2 = new Kassa(store, 2);
        // logger.info("Запускаются 2 потока");
        //System.out.println();

        new Thread(kassa1).start();
        new Thread(kassa2).start();
...
class Store {
    ...
    public void get(int num) {
        synchronized (this) {
            ...
            try {
                fh = new FileHandler("C:\\Users\\mukhamedkhanov\\Desktop\\log\\store.log");
...
class Kassa implements Runnable {

    final Store store;
    final int num;

    Kassa(Store store, int x) {
        this.store = store;
        this.num = x;
    }
